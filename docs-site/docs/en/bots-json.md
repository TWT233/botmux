...
| `nativeSubagentRuntime` | Trae-only native subagent runtime policy. Configure `model` and `reasoningEffort` independently as `{ "mode": "inherit" }` or `{ "mode": "custom", "value": "..." }`; an absent dimension passes through the value from the subagent request. Remove the whole field when both dimensions pass through |
